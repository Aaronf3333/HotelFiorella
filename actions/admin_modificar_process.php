...
if (strtotime($fecha_salida) <= strtotime($fecha_entrada)) {
    // Redirigir de vuelta con un error si la fecha es inválida
...
try {
    ...
    // Obtenemos el ID de la habitación para el recálculo
    $stmt_check = $pdo->prepare("SELECT HabitacionID FROM Reservas WHERE ReservaID = ?");
